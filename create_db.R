...
# this connection creates a new database
con <- DBI::dbConnect(duckdb::duckdb(), dbdir = "coffee_db")

con

loc_query <- "CREATE TABLE locations (
...
# execute sql query
DBI::dbExecute(con, loc_query)

# List tables
DBI::dbListTables(con)

shops_query <- "CREATE TABLE shops (
...
DBI::dbExecute(con, shops_query)

emp_query <- "CREATE TABLE employees (
...
DBI::dbExecute(con, emp_query)

suppliers_query <- "CREATE TABLE suppliers (
    coffeeshop_id INT,
...
DBI::dbExecute(con, suppliers_query)

DBI::dbListTables(con)

# View created table. Remember no data yet.
?dbReadTable

dbReadTable(conn = con,name = "employees")
...
loc_insert_query <- "INSERT INTO locations VALUES(1, 'Los Angeles', 'United States');
INSERT INTO locations VALUES(2, 'New York', 'United States');
INSERT INTO locations VALUES(3, 'London', 'United Kingdom');
"

DBI::dbExecute(con, loc_insert_query)

dbReadTable(conn = con,name = "locations") %>% as_tibble()
con |> 
  DBI::dbReadTable("locations") |> 
  as_tibble() |> View()
...
shops_insert_query <- "INSERT INTO shops VALUES(1, 'Common Grounds', 1);
...
DBI::dbExecute(con, shops_insert_query)


# Insert the first two values where coffeeshop_id = 1 into the suppliers table
suppliers_insert_query <- "INSERT INTO suppliers VALUES(1, 'Beans and Barley', 'Arabica');
INSERT INTO suppliers VALUES(1, 'Cool Beans', 'Robusta');INSERT INTO suppliers VALUES(2, 'Vanilla Bean', 'Liberica');
...
DBI::dbExecute(con, suppliers_insert_query)
...
emp_insert_query <- "INSERT INTO employees VALUES (501559, 'Carson', 'Mosconi', 'cmosconi0@census.gov', '2015/08/29', 'M', 32973, 1);
...
DBI::dbExecute(con, emp_insert_query)

# View the created employees table or modify to view other tables
con |>
  DBI::dbReadTable("employees") |>
  as_tibble() |> View()
